...
> 无状态、零基础设施的港股舆情分析 Agent：自动采集当日雪球/Reddit 评论，LLM 情绪评分，飞书定时推报告。无需数据库和 Redis，直接跑在 GitHub Actions 上。
...
| 变量 | 说明 | 示例 |
|------|------|------|
| `LLM_PROVIDER` | LLM 提供商 | `claude` / `qwen` / `kimi` / `glm` / `minimax` |
...
├── src/
│   ├── collectors/    # 数据采集层（雪球/Reddit/HKEX/Yahoo）
│   ├── analysis/      # LLM 情绪分析（多提供商）
│   ├── scheduler/     # APScheduler 定时任务
│   └── reporters/     # 飞书报告推送
├── tests/             # 单元测试
└── scripts/           # 入口脚本（run.py / run_job.py）
```
