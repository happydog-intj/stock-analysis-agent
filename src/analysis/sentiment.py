...
缓存策略：
  - 使用内存字典对相同内容的分析结果做会话级缓存（避免同批次重复调用 API）
...
class SentimentAnalyzer:
    ...
    def _get_cached(self, content: str) -> dict[str, Any] | None:
        """从内存缓存读取已分析结果。"""
        return self._cache.get(self._cache_key(content))

    def _set_cached(self, content: str, result: dict[str, Any]) -> None:
        """将分析结果写入内存缓存。"""
        ...
        for batch_start in range(0, len(records), batch_size):
            ...
            if uncached_indices:
                ...
                for j, result in enumerate(raw_results):
                    ...
                    # 写入缓存
                    self._set_cached(texts[orig_idx], result)
